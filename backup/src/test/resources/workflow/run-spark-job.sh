...
# spark job jar name.
export SPARK_JOB_JAR=backup-3.0.0-SNAPSHOT-spark-job.jar;
...
export JOB_NAME="s3-backup-${random}"
...
--deploy-mode cluster \
--name ${JOB_NAME} \
--class com.cloudcheflabs.dataroaster.backup.component.S3ToS3Backup \
...
--master ${MASTER} \
--sourceS3PropDefaultFs s3a://mykidong \
--sourceS3PropEndpoint https://smartlife-tenant.minio.cloudchef-labs.com \
--sourceS3PropAccessKey bWluaW8= \
--sourceS3PropSecretKey bWluaW8xMjM= \
--targetS3PropDefaultFs s3a://cloudcheflabs \
--targetS3PropEndpoint https://s3.amazonaws.com \
--targetS3PropAccessKey AKIARJUR6DKSVEB3HZHH \
--targetS3PropSecretKey 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 \
--inputBase /test-parquet \
--outputBase /slbc/backup/test-parquet \
--date NULL;
