...
# wait for minio operator being run.
while [[ $(kubectl get pods -n minio-operator -l name=minio-operator -o jsonpath={..status.phase}) != *"Running"* ]]; do echo "waiting for minio operator being run" && sleep 2; done
