...
import com.cloudcheflabs.dataroaster.apiserver.kubernetes.handler.ArgoHandler;
...
public class ArgoServiceImpl implements ArgoService {
    ...
    private static Logger LOG = LoggerFactory.getLogger(ArgoServiceImpl.class);
    ...
    @Override
    public void createArgo(long namespaceId, long serviceId, String bucket, String s3Endpoint, boolean insecure, String accessKey, String secretKey) {
        K8sServices k8sServices = k8sServicesDao.findOne(serviceId);

        if(!k8sServices.getType().equals(K8sServices.ServiceTypeEnum.WORKFLOW.name())) {
            throw new RuntimeException("It is not type of " + K8sServices.ServiceTypeEnum.WORKFLOW.name());
        }
    ...
    @Override
    public void deleteArgo(long namespaceId, long serviceId) {
        K8sServices k8sServices = k8sServicesDao.findOne(serviceId);
        ...
        Kubeconfig kubeconfig = getAdminKubeconfig(k8sNamespace);
        // remove namespace.
        k8sServices.getWorkflowK8sNamespaceSet().remove(k8sNamespace);
        k8sServicesDao.update(k8sServices);

        ExecutorUtils.runTask(() -> {
            return ArgoHandler.delete(k8sServices,
                    kubeconfig,
                    namespace);
        });
    }
